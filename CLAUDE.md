...
| `backend_args.region_concurrency` | profile YAML | Max concurrent region POSTs in flight across pages (default 16). The detector runs serially in a producer thread; region POSTs from the next page begin as soon as the in-flight count drops below the cap, overlapping detector dead-time with vLLM region work. |
